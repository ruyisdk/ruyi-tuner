 ...
 * implemented if needed, but hopefully this is not neccessary as it has
					  ...
					  char *filename)
{
  ...
  if ((fh = sys->open(sys, filename, MSPACK_SYS_OPEN_READ))) {
    if (!(this->error = mspack_sys_filelen(sys, fh, &filelen))) {
      this->error = cabd_find(this, search_buf, fh, filename, filelen, &cab);
    }
    sys->close(fh);
  }
  else {
		     ...
		     struct mscabd_cabinet_p **firstcab)
{
  ...
  int false_cabs = 0, ishield = 0, length;
  ...
  for (offset = 0; offset < flen; offset += length) {
    ...
    if (offset < 4) {
      for (p = &buf[0]; (p < &buf[length]) && (p < &buf[4]); p++) {
	switch (offset + (p - &buf[0])) {
	case 0: if (*p == 0x49) ishield++; break;
	case 1: if (*p == 0x53) ishield++; break;
	case 2: if (*p == 0x63) ishield++; break;
	case 3: if (*p == 0x28) ishield++; break;
	}
      }
      if (ishield == 4) {
	sys->message(fh, "WARNING; found InstallShield header. "
		         "This is probably an InstallShield file. "
		         "Use the tool I5COMP or I6COMP to unpack it.");
      }
    }
		      ...
		      struct mscabd_cabinet *rcab)
{
  ...
  if (lcab->set_index > rcab->set_index) {
    sys->message(NULL, "WARNING; merged cabinets with the odd Set sequence.");
  }
...
static int cabd_init_decomp(struct mscab_decompressor_p *this, int ct) {
  ...
  case cffoldCOMPTYPE_MSZIP:
    ...
    this->d->state = mszipd_init(&this->d->sys, fh, fh, 1);
    break;
 ...
 * use. either writes data to disk (this->d->fh) with the real
 * sys->write() function, or does nothing with the data when this->d->fh
 * == NULL.  advances this->d->offset
 */
