...
{
    fprintf(stderr , "UNEXPECTED %d: %s; value == %""ll" "x\n",
            recno, reason, value);
    printf("#UNEXPECTED %d %s; value == %""ll" "x\n", recno, reason, value);
}
    ...
    while (read_ndx_record(ndx, &this.r) > 0) {
        ...
        if (mpg) {
            ...
            if (rtv_to_u32(&p) != 0x000001e0) {
                ...
                bfseek(mpg, this.r.stream_position + this.r.video_offset + 56, 0 );
                bfread(buf, 4, 1, mpg);
                p = buf;
            ...
            bfseek(mpg, this.r.stream_position + this.r.video_offset + this.r.audio_offset, 0 );
            bfread(buf, 4, 1, mpg);
            p = buf;
            ...
            if (last.bad_audio) {
                bfseek(mpg, this.r.stream_position + last.r.video_offset + this.r.audio_offset, 0 );
                bfread(buf, 4, 1, mpg);
                p = buf;
                if (rtv_to_u32(&p) != 0x000001c0) {
                    bfseek(mpg, this.r.stream_position + last.r.video_offset + last.r.audio_offset, 0 );
                    bfread(buf, 4, 1, mpg);
                    p = buf;
                    if (rtv_to_u32(&p) != 0x000001c0) {
                        p = buf;
