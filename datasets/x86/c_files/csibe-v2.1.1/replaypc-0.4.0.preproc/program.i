...
int parse_program(unsigned char **pp, struct program * prog)
{
    ...
    if (prog->struct_size) {
        do { if (!( prog->struct_size == 272 )) { fprintf(stderr , "Unexpected %s %s:%d\n", "prog->struct_size == 272", "program.c", 257); if (0 ) (exit(-1)); } } while(0) ;
        do { if (!( prog->unknown1 == 1 )) { fprintf(stderr , "Unexpected %s %s:%d\n", "prog->unknown1 == 1", "program.c", 258); if (0 ) (exit(-1)); } } while(0) ;
        do { if (!( prog->isvalid == 1 )) { fprintf(stderr , "Unexpected %s %s:%d\n", "prog->isvalid == 1", "program.c", 259); if (0 ) (exit(-1)); } } while(0) ;
    }
