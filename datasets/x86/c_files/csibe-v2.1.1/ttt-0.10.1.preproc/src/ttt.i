...
main (int argc, char **argv)
{
  ...
  B = (Board *) ttt_new (players, size, depth);
...
Board *ttt_new (int players, int size, int depth)
{
  ...
  B->copy = ttt_copy;
  B->setup = ttt_setup;
  B->display = ttt_display;
  B->full = ttt_full;
  B->winner = ttt_winner;
  B->valid_move = ttt_valid_move;
  B->move = ttt_move;
  B->unmove = ttt_unmove;
  B->getmove = ttt_getmove;
  B->help = ttt_help;
  B->symbol = ttt_symbol;
  B->coords = ttt_coords;
  ...
  if (MyPE == 0)
    ttt_setup ((Board *)T, players, size, depth);
  ...
  B->board = (int *) malloc (B->squares * sizeof(int));

  B->moves = (int *) malloc (B->squares * sizeof(int));
  B->nummoves = 0;
  B->numpoints = 0;

  for (i = 0; i < B->squares; i++)
    B->board[i] = B->moves[i] = 0;

  if ((rows % 2) == 1)  
    T->center = B->board + ((B->squares-1) / 2);
  else
    T->center = ((void *)0) ;

  if (rows >= 5)
  {
    int numpoints = rows*(cols-3) + cols*(rows-3) + 2*(rows-3)*(cols-3);
    int i, j, point = 0;
    points = (int **) malloc (numpoints * 4 * sizeof(int *));
    board = B->board;
...
Board *ttt_copy (const Board *b)
{
  ...
  rows = B->rows = old->rows;
  cols = B->cols = old->cols;
  B->squares = old->squares;
  B->nummoves = old->nummoves;
  B->numpoints = old->numpoints;
...
void ttt_setup (Board *B, int players, int size, int depth)
{
  Board *T = B;
  const int SIZE = 20;
  ...
  if (size < 3 || size > 6) {
    printf ("What size board? (3-7/?) ");
    while (1 )
    {
      fgets (input, SIZE, stdin );
      input_i = atoi (input);
      if (input_i >= 3 && input_i <= 7)
      {
	T->rows = T->cols = input_i;
	break;
      }
      else if (input[0] == '?')
	setup_help (2, 0);
	...
	printf ("Please enter an integer from 3 to 7, or \"?\" for help: ");
    }

  } else {
    T->rows = T->cols = size;
  }
  squares = (T->rows)*(T->rows);
  ...
  else if (players == 0)
  {
	...
	goto get_moves;
      next:
	if (i == 0) depth1 = T->depth;
	else T->depth2 = T->depth;
      }
      T->depth = depth1;
    } else {
      T->depth = T->depth2 = depth;
    }
  }  
...
int ttt_eval_3 (Board *b)
{
  ...
  {
    sum1 = board[i] + board[i+3] + board[i+6];
    sum2 = board[j] + board[j+1] + board[j+2];
    if (sum1 == 3 || sum2 == 3)
      return 1000000 ;
  ...
  if ((sum1 = board[0]+board[4]+board[8]) == 3 ||
      (sum2 = board[2]+board[4]+board[6]) == 3)
    return 1000000 ;
  else if (sum1 == -3 || sum2 == -3)
    return - 1000000 ;
  
  value = ttt_incrvalue (sum1, sum2, 2, 100, value);
...
int ttt_eval_5 (Board *b)
{
  ...
  for (i = 0, j = 0; i < 5; i++, j += 5) {
    sum1 = board[i] + board[i+5] + board[i+10] + board[i+15] + board[i+20];
    sum2 = board[j] + board[j+1] + board[j+2] + board[j+3] + board[j+4];
    if (sum1 == 5 || sum2 == 5)
      return 1000000 ;
  ...
  if ((sum1 = board[0]+board[6]+board[12]+board[18]+board[24]) == 5 ||
      (sum2 = board[4]+board[8]+board[12]+board[16]+board[20]) == 5)
    return 1000000 ;
  else if (sum1 == -5 || sum2 == -5)
    return - 1000000 ;
...
int ttt_eval_default (Board *b)
{
  ...
  for (i = 0; i < size; i++) {
    ...
    for (j = 0, sum1 = 0, sum2 = 0; j < size; j++) {
      sum1 += board[i*size+j];
      sum2 += board[j*size+i];
    }
    if (sum1 == size)
...
inline int ttt_incrvalue (int sum1, int sum2, int n, int incr, int value)
{
...
inline bool ttt_full (Board *b)
{
  return (b->nummoves == b->squares);
}
...
bool ttt_valid_move (Board *b, int pos)
{
  if (b->board[pos])
    return 0 ;
...
bool ttt_move (Board *b, int player, int pos)
{
  b->board[pos] = player;
...
int ttt_getmove (Board *b, int player, int *x, int *y)
{
  bool moved = 0 ;
  const int SIZE = 200;
  char *input;
  ...
  *x = *y = -1;
  ...
  while (!moved)
  {
    ...
    cmd = parse_command (input, 'a', 'a'+b->cols-1, 1, b->rows, x, y);
    if (cmd == 0 )
      continue;
    if (cmd == 1 ) {
      moved = ttt_valid_move (b, (*x) * b->rows + (*y));
      if (!moved)                          
...
void ttt_help (void)
{
  printf ("\nHelp for Tic Tac Toe:\n"
	  "To make a move, enter the letter and number of the square on "
	  "which you\n  wish to place a piece.  For example, to place a "
	  "piece on the center\n  square, type \"b2\" or \"b 2\" or "
	  "\"2b\" or \"2 b\".\n"
...
char ttt_symbol (Board *b, int x, int y)
{
  int size = b->rows;
  return b->board[x*size+y] == b->X_player ? 'X' : 
           (b->board[x*size+y] == -b->X_player ? 'O' :
	    ' ');
}
...
void ttt_coords (Board *b, int pos, char *buf)
{
  int size = b->rows;
  sprintf (buf, "%c%d", (pos/size)+'a', (pos%size)+1);
}
