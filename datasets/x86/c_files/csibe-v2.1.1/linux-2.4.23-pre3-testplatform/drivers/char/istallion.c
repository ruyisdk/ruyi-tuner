 ...
 */
	...
	printk("%s(%d): sig-> magic=%x rom=%x panel=%x,%x,%x,%x,%x,%x,%x,%x\n",
		"__FILE_", __LINE__, (int) sig.magic, sig.romver, sig.panelid[0],
 ...
 */
	...
	printk("%s(%d): sig-> magic=%x:%x:%x:%x romver=%x amask=%x:%x:%x\n",
		"__FILE_", __LINE__, sig.magic0, sig.magic1, sig.magic2,
...
static int stli_startbrd(stlibrd_t *brdp)
{
	...
	printk("%s(%d): CDK version %d.%d.%d --> "
		"nrdevs=%d memp=%x hostp=%x slavep=%x\n",
		 "__FILE_", __LINE__, hdrp->ver_release, hdrp->ver_modification,
