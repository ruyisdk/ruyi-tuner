 ...
 * READ_BYTES(ptr,n) reads N bytes from the buffer and copies them to ptr.
 *                   note this does NOT use the bit buffer! remember to
 *                   set bits_left to 0 before reading bitstreams again
 *                   
 * ENSURE_BITS(n)    ensures there are at least N bits in the bit buffer.
 *                   it can guarantee up to 17 bits (i.e. it can read in
 *                   16 new bits when there is down to 1 bit in the
 *                   buffer, and it can read 32 bits when there are 0
 *                   bits in the buffer).
 * PEEK_BITS(n)      extracts without removing N bits from the bit buffer
 * REMOVE_BITS(n)    removes N bits from the bit buffer
 *
 * These bit access routines work by using the area beyond the MSB and the
 * LSB as a free source of zeroes when shifting. This avoids having to mask
 * any bits. So we have to know the bit width of the bit buffer variable.
 */
...
#define READ_BYTES(buf, nbytes) do {                                    \
  unsigned int rb_run, rb_todo = (nbytes);                              \
  unsigned char *o_ptr = (buf);                                         \
  while (rb_todo > 0) {                                                 \
    if ((rb_run = i_end - i_ptr)) {                                     \
      if (rb_run > rb_todo) rb_run = rb_todo;                           \
      lzx->sys->copy(i_ptr, o_ptr, rb_run);                             \
      rb_todo -= rb_run;                                                \
      i_ptr += rb_run;                                                  \
      o_ptr += rb_run;                                                  \
    }                                                                   \
...
#define READ_HUFFSYM(tbl, var) do {                                     \
  ...
  hufftbl = &lzx->tbl##_table[0];                                       \
  /* immediate table lookup of [tablebits] bits of the code */          \
  sym = hufftbl[PEEK_BITS(LZX_##tbl##_TABLEBITS)];                      \
  ...
  while (lzx->frame < end_frame) {
	...
	while (this_run > 0) {
	  ...
	  if (main_element < LZX_NUM_CHARS) {
	    ...
	    default:
	      if (match_offset == 3) match_offset = 1;
	      else {
	...
	READ_BYTES(&window[window_posn], this_run);
...
void lzxd_free(struct lzxd_stream *state) {
  struct mspack_system *sys;
  if (state) {
    sys = state->sys;
    sys->free(state->inbuf);
    sys->free(state->window);
    sys->free(state);
  }
}
