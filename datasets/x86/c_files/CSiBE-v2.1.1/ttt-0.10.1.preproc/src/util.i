...
void cmdline_help (char *name)
{
  ...
  printf ("Usage: %s [OPTION]...\n\n"
	  "  -b, --beep\t\tbeep when the computer makes a move\n"
	  "  -d DEPTH\t\tlook ahead DEPTH moves\n"
	  "  -h, --help\t\tdisplay this help and exit\n", name);

  printf ("  -n THREADS\t\tuse THREADS pthreads\n");

  printf ("  -s SQUARES\t\tuse a board size of SQUARES\n"
	  "  -t\t\t\tshow the computer's thinking\n"
	  "  -v, --version\t\tdisplay version information and exit\n"
	  "  -2\t\t\ttwo human players, no computer\n"
	  "  -0\t\t\tno human players, just watch two computers play\n");
...
void get_setup (int players, int *X, int *depth, int *depth2, int *size)
{
  const int SIZE = 20;
  ...
  printf ("What size board? (3-6/?) ");
  while (1 )
  {
    fgets (input, SIZE, stdin );
    input_i = atoi (input);
    if (input_i >= 3 && input_i <= 6)
    {
      *size = input_i;
      break;
    }
      ...
      printf ("Please enter an integer from 3 to 6, or \"?\" for help: ");
  }
...
void setup_help (int mode, int squares)
{
  switch (mode)
  {
    case 1:
      printf ("Type \"y\" to play as X and make the first move.  Type "
	      "\"n\" to play as O and\nlet the computer go first.\n");
      printf ("Do you want to go first? (y/n/?) ");
      break;
    case 2:
      printf ("Enter an integer to indicate the size board to use.  All "
	      "boards are square, so\nentering \"3\" means use a 3x3 board, "
	      "etc.\n");
      printf ("What size board? (3-6/?) ");
      break;
    case 3:
      printf ("Enter an integer to indicate how many moves "
	      "ahead the program\nshould project the game tree in "
	      "searching for its best move.\n");
      printf ("How many moves of look-ahead? (1-%d/?) ",
	      squares);
      break;
  }
  return;
...
void
pos2coords (int pos, int size, int *x, int *y)
{
  *x = pos/size;
  *y = pos%size;
  return;
