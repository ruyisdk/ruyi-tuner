...
static void get_snapshot_callback(unsigned char * buf, size_t len, void * vd)
{
    ...
    if (data->firsttime) {
        ...
        if (end) *end = '\0';
        data->status = strtoul(cur, ((void *)0) , 16);
        if (!end) return;
        do {
            cur = end + 1;
            if (*cur == '#') {
            ...
            *end = '\0';
            ...
            if (strncmp(cur, "guide_file_name=", equal-cur+1) == 0) {
                ...
                data->filesize = strtoul(equal+1, ((void *)0) , 0);
    ...
    bytes_to_read = (( len )<(  (unsigned)(data->filesize - data->bytes_read) )?( len ):(  (unsigned)(data->filesize - data->bytes_read) )) ;
