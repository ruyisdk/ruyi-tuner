...
void dump_u64(char const * tag, u64 value)
{
    fprintf(fp, "%s%-24s\t0x%016""ll" "x %24""ll" "u\n",
            leader, tag, value, value);
}
...
void dump_buffer(char const * tag, u8 const * buffer, size_t len) 
{
    ...
    for (row = 0; row < (len + 16  - 1)/ 16 ; row++) {
        if (row > 0)
            fprintf(fp, "%s                        \t", leader);
        for (column = 0,       byte = row * 16 ;
             column < 16 ;
             column++,         byte++) {
            if (byte < len) {
        ...
        for (column = 0,       byte = row * 16 ;
            ...
            if (byte < len) {
                fprintf(fp, "%c",
                        (((  buffer[byte]  ) & ~0x7f) == 0)   && (__ctype_b[(int) ( ( buffer[byte] ) )] & (unsigned short int)   _ISprint )   ?
                        buffer[byte] : '.');
            } else {
                fprintf(fp, " ");
            }
